...
void FRemWidgetComponentEditorModule::StartupModule()
{
    ...
    OnObjectReplacedHandle = FCoreUObjectDelegates::OnObjectsReplaced.AddRaw(this,
        &FRemWidgetComponentEditorModule::OnObjectReplaced);
    ObjectModifiedHandle = FCoreUObjectDelegates::OnObjectModified.AddRaw(this,
        &FRemWidgetComponentEditorModule::OnObjectModified);
}
...
void FRemWidgetComponentEditorModule::RegisterCustomization()
{
    ...
    auto bSignificantlyChanged            = false;
...
void FRemWidgetComponentEditorModule::OnObjectReplaced(
    const TMap<UObject*, UObject*>& ReplacementObjectMap) const
{
...
void FRemWidgetComponentEditorModule::OnObjectModified(UObject* Object)
{
    ...
    else
    {
            ...
            && PreviewWidget && PreviewWidget->IsDesignTime())
        {
            ...
            {
                Rem::Object::SetTimerForThisTick(PreviewWidget, Rem::FTimerDelegate::CreateWeakLambda(
                    WidgetBlueprint.Get(), [this]
                    {
...
// ReSharper disable once CppMemberFunctionMayBeStatic
void FRemWidgetComponentEditorModule::UpdateSoftObjects(
    const TWeakObjectPtr<const UBaseWidgetBlueprint> InWidgetBlueprint) const
{
    ...
    Rem::WidgetComponent::ForeachUserWidgetComponent(DefaultObject,
        [&](URemWidgetComponentBase** ObjectMemberPtr, int32)
        {
            ...
            Rem::Property::IteratePropertiesOfType<FSoftObjectProperty>(ComponentBase->GetClass(),
                ...
                [&](const Rem::TNotNull<const FSoftObjectProperty*> SoftObjectProperty, void* InContainer)
                {
                    ...
                    if (const auto* SoftObject = SoftObjectPtr->Get();
                        SoftObject && SoftObject->GetFName() != FName(
                            *Rem::GetObjectNameFromSoftObjectPath(SoftObjectPtr->ToSoftObjectPath())))
                    {
